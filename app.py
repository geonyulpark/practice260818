...
# ==============================================================
    ...
    st.markdown("""
    <style>
    div[class*="st-key-detail_page_date"],
    div[class*="st-key-detail_page_issuer"] {
    ...
    if "GOOGLE_SHEET_ID" not in st.secrets or "gcp_service_account" not in st.secrets:
        ...
        if day_spread.empty and as_of_trigger.empty and as_of_winus.empty:
            ...
            if options_map:
                ...
                # 다른 페이지(예: 수요예측 일정 팝업)에서 특정 발행사를 지정해서 넘어온 경우,
                # 그 발행사가 오늘 목록에 실제로 있을 때만 기본 선택값으로 반영 (없으면 조용히 무시)
                _pending_issuer = st.session_state.pop("bsch_target_issuer", None)
                if _pending_issuer:
                    _match = [v for v in options_map.values() if normalize_issuer_name(v) == normalize_issuer_name(_pending_issuer)]
                    if _match:
                        st.session_state["detail_page_issuer"] = _match[0]
                ...
                if pick_issuer:
                    ...
                    st.markdown(f"**채권 스프레드{date_note}**")
                    ...
                    if not as_of_trigger.empty and "원본발행사명" in as_of_trigger.columns:
                        ...
                        st.markdown(f"**신용등급 변동 트리거 (해당 시점 기준)**")
                    ...
                    if not as_of_winus.empty and "발행사명" in as_of_winus.columns:
                        ...
                        st.markdown(f"**익스포저 현황 (해당 시점 기준)**")
                    ...
                    with st.expander("DART 기업개황 및 재무제표 (필요시 펼쳐서 조회)", expanded=False):
                        if not DART_API_KEY:
                            st.caption("DART API 키가 설정되어 있지 않아 이 정보를 조회할 수 없습니다.")
                        else:
                            st.caption(
                                "⚠️ 재무제표 상세 조회는 한 번에 최대 15회 정도 DART를 호출합니다. "
                                "요청 과다로 인한 일시적 차단을 막기 위해 버튼을 눌러야 조회됩니다."
                            )
                            run_dart_detail = st.button("DART에서 기업개황·재무제표 조회", key="dart_detail_fetch_btn")

                            if not run_dart_detail:
                                st.caption("버튼을 누르면 아래에 결과가 표시됩니다.")
                            else:
